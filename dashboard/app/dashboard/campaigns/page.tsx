...
import { getCampaignBreakdown, getDateRange, windowFrom } from '@/lib/queries';
...
export default async function CampaignsPage({ searchParams }: { searchParams: { d?: string; from?: string; to?: string } }) {
  const w = windowFrom(searchParams);
  const campaigns = await getCampaignBreakdown(w);
  ...
  const columns: Column<any>[] = [
    { key: 'campaign_name', label: 'Campaign', render: renderName },
  ...
  return (
    <div>
      ...
      <div className="p-5">
        <div className="text-xs text-slate-500 mb-4">{campaigns.length} campaigns · {w.start} to {w.end}</div>
        <DataTable rows={campaigns} columns={columns} searchKeys={['campaign_name', 'objective']} searchPlaceholder="Search" defaultSortKey="spend" />
      </div>
    </div>
  );
}
