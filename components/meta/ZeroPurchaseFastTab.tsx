...
export function ZeroPurchaseFastTab() {
  ...
  async function load(force = false) {
    ...
    try {
      const res = await fetch(
        `/api/meta-os-fast?view=zero_purchase&threshold=${threshold}${force ? `&force=${Date.now()}` : ""}`,
        { cache: "no-store" }
      );

      const json = await res.json();

      if (!res.ok) throw new Error(json?.error || "Failed to load Zero Purchase data");
    ...
    <div className="grid gap-3">
        ...
        <div className="flex flex-col gap-3 lg:flex-row lg:items-end lg:justify-between">
          <div>
            <p className="text-[11px] font-black uppercase tracking-[0.16em] text-red-600 dark:text-red-300">
              Fast Zero Purchase Control
            </p>
            <h1 className="mt-1 text-2xl font-black">Live Zero-Purchase Ads</h1>
            <p className="mt-1 text-sm opacity-60">
              Server-side cached view. Only active-yesterday ads are processed, then lifetime zero-purchase waste is shown.
            </p>
          </div>
      ...
      <section className="rounded-xl border border-current/10 bg-current/[0.025] p-4">
        <div className="grid gap-2 md:grid-cols-5">
          <Kpi label="Zero Ads" value={String(summary.count)} tone={summary.count > 0 ? "red" : "green"} />
          <Kpi label="Lifetime Waste" value={money(summary.totalSpend)} tone={summary.totalSpend > 0 ? "red" : "green"} />
          <Kpi label="Yesterday Waste" value={money(summary.yesterdaySpend)} tone={summary.yesterdaySpend > 0 ? "red" : "green"} />
          <Kpi label="Active Ads" value={String(summary.activeAdCount)} />
          <Kpi label="Rows Used" value={String(summary.rawRowsUsed)} />
        </div>

        {payload?.cached ? (
          <p className="mt-3 text-xs opacity-60">Loaded from 10-minute server cache.</p>
        ) : null}
        ...
        <div className="flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-sm opacity-60">Default is ₹3,000. You can use preset or custom value.</p>
            </div>
          </div>
      ...
      <section className="rounded-xl border border-current/10 bg-current/[0.025]">
        <div className="flex items-center justify-between gap-3 border-b border-current/10 px-4 py-3">
          <div>
            ...
            <p className="mt-1 text-sm opacity-60">
              Showing ads live yesterday with lifetime spend above {money(threshold)} and zero lifetime purchases.
            </p>
          </div>
        ...
        <div className="divide-y divide-current/10">
          {loading && !items.length ? (
            <div className="p-5 text-sm opacity-60">Loading fast Zero Purchase view...</div>
          ) : null}
              ...
              <div className="grid gap-3 px-4 pb-4 lg:grid-cols-[1fr_1fr_440px]">
                <InfoBox
                  ...
                  lines={[
                    ...
                    `It still spent yesterday, so it is active, not old/paused.`,
          ...
          {!loading && !items.length ? (
            <div className="p-5">
              ...
              <p className="mt-1 text-sm opacity-60">Try lowering the lifetime spend threshold.</p>
            </div>
          ) : null}
        </div>
      </section>
